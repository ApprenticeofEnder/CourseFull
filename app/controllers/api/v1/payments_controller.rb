    ...
    class PaymentsController < Api::V1::ApplicationController
      ...
      def create
        ...
        api_v1_payment_params[:products].each do |product|
          Rails.logger.info(format('Account %p is attempting to purchase %d items of product %p', @api_v1_user.id,
                                   product[:quantity], product[:stripe_id]))
          @line_items.push({ price: product[:stripe_price], quantity: product[:quantity] })
        end

        unless @line_items.length.positive?
          render json: { error: 'You cannot check out with an empty cart!', error_type: 'empty_cart' },
                 status: :bad_request
          return
        end
      ...
      def api_v1_payment_params
        params.require(:payment).permit(products: %i[stripe_id stripe_price quantity])
      end
